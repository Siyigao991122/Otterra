...
- openingsMeters (strongly recommended): array of { id?, type ("door"|"window"), x, y, width, height?, sillHeight? } for all doors and windows visible on the plan. Position (x,y) is the CENTER of the opening in the same plan coordinate system and METERS scale as footprintPolygonMeters. Width in meters. Height: default 2.1m for doors, 1.2m for windows. SillHeight: 0m for doors, 0.9m typical for windows. Include exterior doors, interior doors, and all windows you can see on the plan. Door symbols appear as arc/swing marks; window symbols appear as parallel lines across a wall gap.
- roomZonesMeters (strongly recommended): array of { id?, label, type, polygon: [{x,y}], centroid?: {x,y} } for each identifiable room or space. Use the EXACT same plan coordinate system and METERS scale. Label is the room name (e.g. "Living Room", "Master Bedroom"). Type must be one of: "living_room", "bedroom", "bathroom", "kitchen", "dining", "hallway", "office", "balcony", "storage", "garage", "other". Polygon traces the INTERIOR boundary of the room (inside the wall faces). Centroid is the center point of the room. List every named or implied space.
